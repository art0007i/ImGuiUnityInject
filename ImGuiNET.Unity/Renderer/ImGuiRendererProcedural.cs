...
namespace ImGuiNET.Unity
{
    ...
    sealed class ImGuiRendererProcedural : IImGuiRenderer
    {
        ...
        public ImGuiRendererProcedural(DearImGui resources, TextureManager texManager)
        {
            if (SystemInfo.graphicsShaderLevel < 45)
                throw new System.Exception("Device not supported");

            _shader = resources.procedural;
            _texManager = texManager;
            _texID = Shader.PropertyToID(resources.tex);
            _verticesID = Shader.PropertyToID(resources.vertices);
            _baseVertexID = Shader.PropertyToID(resources.baseVertex);
        }
